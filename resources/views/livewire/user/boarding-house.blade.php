                    ...
                    <div class="col-md-12">
                        <div class="card-profile-stats">
                            <div class="d-flex justify-content-center align-items-center">
                                @for ($i = 1; $i <= 5; $i++)
                                    <i class="fa fa-star {{ $i <= $ratings ? 'text-primary' : '' }}"
                                        aria-hidden="true"></i>
                                @endfor
                            </div>
                        ...
                        <div class="col-md-12" style="height: 100%">
                            <div class="card" style="height: 100%">
                                ...
                                <div class="card-body" id="chatbox" style="height: 100%; overflow-y: auto;">
                                    {{-- @dd($messageReviews) --}}
                                    @forelse ($messageReviews as $messageReview)
                                        <div class="message received d-flex" wire:key="{{ $messageReview->id }}">
                                            <img src="{{ asset('storage/images/' . $messageReview->getUser->profileImage) }}"
                                                alt="User Avatar" class="avatar mr-1">
                                            <div class="message-content">
                                                <strong>{{ $messageReview->getUser->firstName }}
                                                    {{ $messageReview->getUser->lastName }}</strong>
                                                <p
                                                    style="padding: 5px; background-color: rgb(238, 237, 237); border-radius: 10px;">
                                                    {{ $messageReview->message }}</p>
                                                <p style="font-size:11px; text-align:right; margin-top:-10px;">
                                                    {{ $messageReview->created_at->shortAbsoluteDiffForHumans() }}</p>
                                            </div>
                                        </div>
                                    @empty
                                    ...
                                    @if (count($messageReviews) < $messageTotalReviews)
                                        <p class="text-primary" wire:click="loadMore"
                                ...
                                @if ($messageApprovedCount > 0)
                                    <div class="card-footer">
                                        <div class="input-group">
                                            <textarea type="text" id="message" wire:model="message" class="form-control" placeholder="Type your review..."></textarea>
                                            <div class="input-group-append">
                                                <button wire:click="sendMessage"
                                                    class="btn btn-primary {{ $messageApprovedCount > 0 ? '' : 'disabled' }}"
                                                    {{ $messageApprovedCount > 0 ? '' : 'disabled' }}>
